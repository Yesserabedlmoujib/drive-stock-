...
import { downloadBonPDF } from "@/lib/bonLivraisonPdf";
import {
  ChevronDown,
  ChevronUp,
  Download,
...
export default function History() {
  ...
  const [deleteBon, setDeleteBon] = useState<BonDeLivraison | null>(null);
  ...
  const handleDownload = async (bon: BonDeLivraison) => {
    try {
      const profile =
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await bonDeLivraisonService.delete(deleteBon.id);
    ...
    <div className="page-container">
      ...
      {bons.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">{t("delivery_history_empty")}</p>
        </div>
      ) : (
      ...
      <AlertDialog open={!!deleteBon} onOpenChange={() => setDeleteBon(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("delete_delivery_title")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("delete_delivery_confirm_message")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
