...
import type { BonDeRetourItem, Product } from "@/db/types";
...
export default function BonR() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const itemsCanonical: BonDeRetourItem[] = selectedProducts.map((sp) => {
      ...
      const bonNumber = `BR-${new Date().getFullYear()}-${String(
        Math.floor(Math.random() * 9000) + 1000,
      )}`;

      await bonDeRetourService.create({
        number: bonNumber,
        lieu: lieu.trim(),
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-xl border border-border p-4 md:p-6">
            ...
            <Button
              ...
              disabled={saving || !lieu.trim() || selectedProducts.length === 0}
